...
|--------|------------|-------------------|----------------------------|------------|
| `NEXT_PUBLIC_SITE_KEY` | `apps/demo-restaurant-c` | `kushi` · `tokiwa-sushi-bowls` · `hungry-wombat` | `tokiwa-sushi-bowls` | `data/<key>.json`（`SITES` に登録必須） |
...
| — | `apps/demo-salon-b` | — | — | 環境変数なし。`data/site.json` を直接 import。 |
...
# 飲食：hungry-wombat テンプレ（ルートショートカット可）
...
| `apps/demo-salon-b` | サロン文面サンプル（`site.json` 固定。環境変数なし） |
